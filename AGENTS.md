...
- `experiments/`: research scripts and their reproduction guide; not
  wired into the CLI. `experiments/_archived/routing_encoder.py` is
  generated by `build_shim.py` and gitignored.
- `experiments/score_shadow_model.py`: inference-only JSONL scorer for the
  retained frozen and LoRA artifacts; it returns scores and provenance, never
  authorization decisions.
- `experiments/prepare_external_eval.py`: deterministic PromptShield and SEP
  acquisition and projection at pinned revisions.
