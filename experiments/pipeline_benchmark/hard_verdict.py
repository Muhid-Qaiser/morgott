...
SLICE_FIELDS = ("dataset", "input_channel", "source")
...
def choose_joint_provider(
    ...
    eligibility = joint_provider_eligibility(provider_profiles)
    eligible = [
        value
        for value in provider_profiles
        if eligibility[value["provider"]]["eligible"]
    ]
    if not eligible:
        return None
    return min(
...
def joint_provider_eligibility(
    provider_profiles: list[dict[str, Any]],
    ...
    result = {
        value["provider"]: {
            "eligible": False,
            "reason": "reliability or required-profile gate failed",
            "overall_recall_delta": None,
            "slice_recall_deltas": {},
        }
        for value in provider_profiles
    }
    ...
    for value in comparable:
        balanced = value["profiles"]["balanced"]
        overall_delta = best_recall - balanced["metrics"]["recall"]
        slice_deltas = {
            name: best - balanced["slice_recall"][name]
            for name, best in best_slices.items()
            if balanced["slice_recall"][name] < best - MAX_SLICE_RECALL_DELTA
        }
        eligible = (
            balanced["metrics"]["recall"] >= best_recall - MAX_RECALL_DELTA
            and not slice_deltas
        )
        result[value["provider"]] = {
            "eligible": eligible,
            "reason": (
                None if eligible else "overall or declared-slice recall delta exceeded"
            ),
            "overall_recall_delta": overall_delta,
            "slice_recall_deltas": dict(sorted(slice_deltas.items())),
        }
    return result
...
def _select(output: Path) -> tuple[dict[str, Any] | None, str]:
    ...
    selection_status = "selected" if selected is not None else "no_eligible_provider"
    result = {
        "schema_version": 3 if selected is not None else 4,
        "advisory_only": True,
        ...
        "strict_provider_eligibility": joint_provider_eligibility(provider_profiles),
        "profiles": (
            selected["profiles"]
            if selected is not None
            else {name: None for name in metrics.PROFILE_CONSTRAINTS}
        ),
        "selected_profile_infeasibility": (
            selected["profile_infeasibility"]
            if selected is not None
            else {
                "provider_selection": {
                    "reason": (
                        "no strict hard-verdict provider met the overall and "
                        "declared-slice quality gates"
                    )
                }
            }
        ),
...
def analyze(output: Path) -> str:
    ...
    if selection.get("selection_status") == "no_eligible_provider":
        return "complete: no eligible strict hard-verdict provider"
