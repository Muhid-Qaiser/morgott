...
def install_matched_pairs_hook(fraction: float = 1.0,
                               ranking: bool = False) -> int:
    ...
    pairs_dir = REPO_ROOT / "artifacts" / "matched_pairs"
    rows = []
    for path in sorted(pairs_dir.glob("pairs_*.jsonl")):
        if ".log." in path.name:
            continue
        with path.open(encoding="utf-8") as handle:
            for index, line in enumerate(handle):
                ...
                if pair.get("channel") != "direct_user":
                    continue
                group = f"genpair:{path.stem}:{index}"
                for half, label in (("benign", 0), ("attack", 1)):
                    ...
                    rows.append({
                        ...
                        "security_label": "benign" if label == 0 else
    ...
    if fraction < 1.0:
        ...
        keep = set(_random.Random(42).sample(
            groups, max(1, int(len(groups) * fraction))))
    ...
    print(f"matched-pairs hook installed: +{len(rows)} training rows "
...
def main() -> int:
    ...
    parser.add_argument("--model-revision", default=archived.MODEL_REVISION)
    ...
    if args.matched_pairs:
        install_matched_pairs_hook(
            fraction=args.pairs_fraction, ranking=args.pair_ranking > 0
        )
    suffix = ""
    if args.matched_pairs:
        suffix = "_pairs"
    ...
    print(f"model      : {args.model_id} @ {args.model_revision[:12]}")
