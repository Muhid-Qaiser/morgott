...
then-registered advisory shadow `mmbert-lora-full-s42` at the direct-user pass floor of `0.2`:
...
Their retired 512-token scorer remains in Git history; the live helper now rebuilds only
the pinned matched-pair projection. The historical scores are advisory and `decision`
remains `allow`.
